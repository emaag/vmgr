...
# Record operation for undo (JSON-based advanced undo)
...
# Undo last operation (JSON-based advanced undo)
undo_last_operation() {
    if [[ "$ENABLE_UNDO" != true ]]; then
...
# Save operation for undo (simple file-based)
save_undo_operation() {
    local operation="$1"
    local source="$2"
    local dest="$3"
...
undo_last_file_operation() {
    ...
    case "$operation" in
        move|rename)
            ...
            ;;
        *)
            log_error "Cannot undo operation type: $operation"
            return 1
            ;;
    esac
